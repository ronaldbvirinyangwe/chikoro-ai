...
const buildSystemInstruction = (grade, curriculum) => {
  return `You are Chikoro AI, a personalised AI tutor for Zimbabwean students. Your core identity and instructions, which you must follow at all times, are:
1.  **Persona**: You are a friendly, patient, and encouraging tutor.
2.  **Student Level**: Your current student is in ${grade}. You MUST adapt all explanations, examples, and language to be appropriate for this grade level.
3.  **Contextualize**: Use examples and analogies relevant to the Zimbabwean context (e.g., local places, culture, currency, etc.) to make concepts relatable.
4.  **Language**: You can mix English and Shona/Ndebele where it feels natural, especially if the user does.
5.  **THE MOST IMPORTANT RULE**: You MUST NOT give direct answers. Your primary method of teaching is the Socratic method. Guide the student to the answer by asking leading questions.
6.  **Curriculum**: Your teaching should align with the ${curriculum} curriculum.
7.  **Adaptive Learning**: If a student is struggling, simplify the concept. If they are grasping it well, introduce more complex ideas.
8.  **Character**: Never break character. Do not mention that you are an AI model.`;
};
...
apiRouter.post("/chat/conversation", authenticateUser, async (req, res) => {
  try {
    ...
    // --- Step 2: Triage (Decision to search) ---
    // This prompt is more robust and tells the AI "how" to think.
    const triagePrompt = `You are a helpful routing assistant. Your task is to analyze the user's prompt and decide if a real-time web search is required. The user is a ${grade} student in Zimbabwe.
    Think step-by-step:
    1.  Is the user asking for a current event (post-2023), a specific statistic, or information about a very recent discovery?
    2.  Is it a factual question that is unlikely to be in your general training data (e.g., "What was the price of bread in Harare yesterday?")?
    3.  Or, is it a general academic question (e.g., "What is photosynthesis?"), a math problem, or a conversational request?

    Based on your analysis, respond ONLY with a valid JSON object in the format: {"action": "search" | "answer", "query": "a search-engine-friendly query string"}.
    - Use "search" if a web search is needed.
    - Use "answer" if you can answer from general knowledge.
    User Prompt: "${prompt}"`;

    // It's safer to wrap this in a try/catch in case the model returns invalid JSON
    let triageDecision;
    try {
        triageDecision = await getOllamaJson([
            { role: "user", content: triagePrompt }
        ], true, OLLAMA_MODEL_NAME);
    } catch (e) {
        console.error("Triage model returned invalid JSON or failed.", e);
        // Default to answering directly if triage fails
        triageDecision = { action: "answer", query: prompt };
    }


    // --- Step 3: Prepare Final Prompt (Unified Logic) ---
    const systemInstruction = buildSystemInstruction(grade, studentCurriculum);
    let userMessageContent;

    if (triageDecision.action === "search") {
      console.log(`AI decided to search. Query: "${triageDecision.query}"`);
      const searchResults = await performGoogleSearch(triageDecision.query);

      // **THE CRITICAL FIX IS HERE**
      // The prompt now REINFORCES the persona instead of contradicting it.
      userMessageContent = `A student has asked me the following question: "${prompt}".

      I have performed a web search to get up-to-date information. Here are the search results:
      """
      ${searchResults}
      """

      Now, using these search results as your knowledge base, please help me guide the student to the answer. Remember to follow all of your core instructions: use the Socratic method, adapt to their grade level, use Zimbabwean examples, and DO NOT give a direct answer. Start the conversation with a guiding question.`;

    } else {
      console.log("AI decided to answer directly.");
      userMessageContent = prompt; // The original prompt is sufficient here.
    }
    ...
    // --- Step 4: Stream the final response to the client ---
    return streamOllamaResponse(messagesForFinalAnswer, res);
...
apiRouter.get("/chat/dynamic-cards", authenticateUser, async (req, res) => {
  try {
    ...
    const GENERATION_MODEL = "gemma-3:1b";
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ error: "Missing required history parameters" });
      }

      const studentsCollection = db.collection("students");

      // Prepare the chat entries to be saved
      const userEntry = {
        type: "user",
        message: userMessage,
        timestamp: new Date(),
      ...
      const modelEntry = {
        type: "model", // Use 'assistant' or 'model' consistently with your frontend
        message: modelResponse,
        timestamp: new Date(),
      };

      // Use MongoDB's $push operator with $each to add both messages to the
      // correct subject array within the chatHistory object.
...
apiRouter.post("/tests/grade", authenticateUser, async (req, res) => {
  ...
  const profileId = req.user.id;
  const GRADING_MODEL = "gemma-3:1b";
...
apiRouter.post("/quizzes/:quizId/submit", authenticateUser, async (req, res) => {
    try {
        ...
        const GRADING_MODEL = "gemma-3:1b"; // Ensure you have this constant defined
...
apiRouter.get("/quizzes/teacher", authenticateUser, async (req, res) => {
    console.log("Teacher Quizzes Route: Entered."); // ADDED LOG
    try {
        // First, ensure the authenticated user is a teacher.
        // req.user.id comes from the authenticateUser middleware, parsed from the JWT.
        const teacher = await db.collection("teachers").findOne({ _id: new ObjectId(req.user.id) });
        
        console.log("Teacher Quizzes Route: Teacher found in DB?", !!teacher); // ADDED LOG
        console.log("Teacher Quizzes Route: User ID from token:", req.user.id); // ADDED LOG

        if (!teacher) {
            console.log("Teacher Quizzes Route: Access denied (403) - User is not a registered teacher."); // ADDED LOG
            return res.status(403).json({ error: "Access denied. User is not a registered teacher." });
        }

        const teacherId = new ObjectId(req.user.id);
        console.log("Teacher Quizzes Route: Using teacherId for aggregation:", teacherId); // ADDED LOG

        // Use an aggregation pipeline to efficiently get quizzes and their submission counts.
        const quizzesWithCounts = await db.collection("quizzes").aggregate([
            // Stage 1: Match only the quizzes created by the logged-in teacher.
            {
                $match: { teacherId: teacherId }
            },
            // Stage 2: Join with the students collection to find submissions.
            {
                $lookup: {
                    from: "students", // The collection to join with.
                    let: { quiz_id: "$_id" }, // Variable for the quiz's ID.
                    pipeline: [
                        // Find students who have a test result matching this quiz ID.
                        { $match: { "tests.quizId": "$$quiz_id" } }
                    ],
                    as: "submissions" // Name of the new array field with matching students.
                }
            },
            // Stage 3: Add a new field for the count of submissions.
            {
                $addFields: {
                    submissionCount: { $size: "$submissions" }
                }
            },
            // Stage 4: Project the final fields to send to the frontend.
            {
                $project: {
                    _id: 0, // Exclude the default _id
                    quizId: "$_id", // Rename _id to quizId
                    title: 1, // Include title
                    subject: 1, // Include subject
                    grade: 1, // Include grade
                    createdAt: 1, // Include creation date
                    submissionCount: 1 // Include our new submissionCount
                }
            },
            // Stage 5: Sort by most recently created.
            {
                $sort: { createdAt: -1 }
            }
        ]).toArray();

        console.log(`Teacher Quizzes Route: Successfully fetched ${quizzesWithCounts.length} quizzes.`); // ADDED LOG
        res.json(quizzesWithCounts);

    } catch (error) {
        console.error("Teacher Quizzes Route: Error fetching teacher's quizzes:", error); // Make sure this logs the full error object
        // Log the error response if available for more details
        if (error.response) {
            console.error("Teacher Quizzes Route: Error response data:", error.response.data);
            console.error("Teacher Quizzes Route: Error response status:", error.response.status);
            console.error("Teacher Quizzes Route: Error response headers:", error.response.headers);
        }
...
apiRouter.get("/quizzes/:quizId/submissions", authenticateUser, async (req, res) => {
    try {
        ...
        // Authorization: Ensure the user is a teacher and owns this quiz.
        ...
        // Find all students who have submitted this specific quiz.
        const studentsWithSubmissions = await db.collection("students").find({
            "tests.quizId": new ObjectId(quizId)
        }).project({
            name: 1, // Get student's name
            tests: 1   // Get the tests array
        }).toArray();

        // Process the data to create the final report.
        const submissions = studentsWithSubmissions.map(student => {
            // Find the specific test submission from the student's tests array.
            const submission = student.tests.find(t => t.quizId.toString() === quizId);
            
            // Generate mock AI feedback if it doesn't exist
            // In a real scenario, this would be generated upon submission
            const aiFeedback = submission.details?.summary || {
                overallFeedback: "This submission has not been processed by the AI grader.",
                strengths: [],
                areasForImprovement: []
            };
